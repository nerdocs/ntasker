...
A missing summary is generated by a **second background loop** (`ntasker.triage.summary_worker`, one project every
3 s, idle poll every 60 s): one `claude -p` call per project that reads the top of `CLAUDE.md` and `README.md` plus the
top-level directory listing (so a repo without docs still gets a sentence). A project whose call fails is skipped for
the rest of the server's run -- it would otherwise stay first in the queue and spin -- and is retried on the next start
...
| GET | `/api/inbox` | `{items, tasks, summaries: {done, total}}` -- notes oldest first, proposals newest first |
...
- `src/ntasker/triage.py` -- catalog, summaries, prefix, prompt, the call, validation, `tick`/`worker` and
