...
The catalog's summaries are generated by a second loop (:func:`summary_worker`,
one project per tick) instead of inline, so a note is never queued behind a
catch-up run over every project that has none yet.
...
SUMMARY_TICK = 3.0
"""Seconds between two summary-worker ticks while summaries are missing."""

SUMMARY_IDLE_TICK = 60.0
"""Seconds the summary worker waits once the catalog is complete."""
...
def summary_progress() -> dict[str, int]:
    ...
    """
    cat = catalog()
    return {"done": sum(1 for _n, summary in cat if summary), "total": len(cat)}


_summary_failed: set[str] = set()
"""Projects whose summary call failed in this process -- skipped by the worker.

A failing project stays first in the missing list, so without this the worker
would spin on it forever. Retried on the next server start or on demand
through *Regenerate*.
"""


def summary_tick() -> bool:
    """Generate the next missing project summary. ``True`` when one was tried."""
    pending = [n for n, summary in catalog() if summary is None and n not in _summary_failed]
    if not pending:
        return False
    try:
        summarize_project(pending[0])
    except TriageError:
        _summary_failed.add(pending[0])
    return True


async def summary_worker() -> None:
    """Fill the catalog's missing summaries in the background, one per tick.

    Deliberately outside :func:`tick`: generating dozens of summaries inline
    took minutes, and every inbox note sat in ``pending`` meanwhile. The
    triage now runs against the catalog as it stands -- a project without a
    summary is offered by name -- while this loop closes the gap.
    """
    while True:
        busy = False
        with contextlib.suppress(Exception):
            if get_triage_enabled():
                busy = await asyncio.to_thread(summary_tick)
        await asyncio.sleep(SUMMARY_TICK if busy else SUMMARY_IDLE_TICK)
...
def triage_text(text: str) -> dict:
    """Catalog -> prefix -> ``claude -p`` -> validated proposal.

    The catalog is taken as it stands: a project whose summary has not been
    generated yet is offered by name alone (:func:`summary_worker` fills it in
    the background), so a note is never blocked on a summary run. Returns the
    :func:`parse_result` dict plus ``raw`` (the note) and ``prefix`` (whether
    the note named the project).
    """
    cat = catalog()
