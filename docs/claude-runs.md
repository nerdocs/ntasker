...
it keeps running when you press **Back** or even reload the page. While a session is live the task's robot button
shows a **spinner**. Re-opening the run view reattaches: ntasker replays the recent output buffer to reconstruct the
screen, then streams live again. Several tasks can run at once, each with its own button state.
...
interactive process is torn down.
...
* Endpoints: `GET /api/claude/status` (CLI + PTY available?), `GET /api/claude/sessions` (live task ids, for the busy
  indicators), `GET /api/tasks/<id>/claude-run/defaults` (guessed cwd + `/task <id>` seed).
