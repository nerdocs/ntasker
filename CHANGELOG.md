...
## [Unreleased]
Sessions: carry on with a terminal conversation -- `/task` reports its id, `ntasker adopt`, and a board pick-up dialog.
...
Runs: `ntasker run <id> [--open]` -- the board's run button from a terminal; prints the lane position + run URL.
...
Fork migration: `project_categories` table folds into `project_groups` on boot; guide in docs/migrating-from-fork.md.
Workspace plugin: team (Claude Code subagents), skills, knowledge base, documents -- page, sidebar, file viewer.
